...
_BACKBONES_OVERRIDE = {
    "wideresnet50": lambda: (
        # Legacy (deprecated in torchvision>=0.13):
        # models.wide_resnet50_2(pretrained=True)
        models.wide_resnet50_2(weights=models.Wide_ResNet50_2_Weights.DEFAULT)
    ),
    "wideresnet101": lambda: (
        # Legacy (deprecated in torchvision>=0.13):
        # models.wide_resnet101_2(pretrained=True)
        models.wide_resnet101_2(weights=models.Wide_ResNet101_2_Weights.DEFAULT)
    ),
}


def load(name):
    if name in _BACKBONES_OVERRIDE:
        try:
            return _BACKBONES_OVERRIDE[name]()
        except Exception:
            # Fallback for older torchvision versions (no Weights enums).
